  ...
  <div class="col-md-12">
    <div class="card">
      <div class="card-header">
        <h2 class="card-title">{{ Str::ucfirst($status) }} Orders</h2>
      </div>
      <div class="card-body">
        @if ($orders && count($orders) > 0)
        <table class="table table-striped" id="view_orders_tb">
          <thead>
            <tr>
              <th>#</th>
              <th>Order Date</th>
              ...
              <td>
                ...
                <div class="dropdown-menu">
                  <a class="dropdown-item text-danger"
                    href="{{ route('admin.orders.view_details', ['id' => base64_encode($order->id)]) }}">View</a>
                  <a onclick="changeOrderStatus(`{{base64_encode($order->id)}}`,`{{$order->order_status}}`)"
                    class="dropdown-item text-success" href="javascript:void(0)">Change Status</a>
                  <a onclick="changePaymenttatus(`{{base64_encode($order->payment_status)}}`)"
                    class="dropdown-item text-warning" href="javascript:void(0)">Payment</a>
