...
@section('title', 'Edit Discounted Product')

@section('content')
<div class="container">
    <div class="card">
        <div class="card-header">
            <h1 class="card-title">Edit Discounted Product</h1>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col-md-12">
                    <form action="{{ route('admin.discount.discounted-product.update', $discount->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="name" name="name" value="{{ $discount->product->name }}" readonly>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="status" name="status" required>
                                        ...
                                        <option value="1" {{ ($discount && $discount->status) ? 'selected' : '' }}>On</option>
                                        <option value="0" {{ ($discount && !$discount->status) ? 'selected' : '' }}>Off</option>
                                    </select>
                                </div>
                            </div>
                        </div>

                        <button type="submit" class="btn btn-primary">Update Discounted Product</button>
                    </form>
                </div>
            </div>
        </div>
